...
const DEFAULT_MAX_PACKET_LEN: uint = 10 * 1024 * 1024
...
proc newWebSocket*(
  req: Request,
  protocol: string = "",
  maxPacketLength = DEFAULT_MAX_PACKET_LEN,
  ...
  try:
    ...
    ws.maxPacketLength = maxPacketLength
...
proc newWebSocket*(url: string, protocol: string,
    ...
    Future[WebSocket] {.async.} =
  return await newWebSocket(url, @[protocol], maxPacketLength = maxPacketLength)
...
proc recvFrame(ws: WebSocket): Future[Frame] {.async.} =
  ...
  # Read the data.
  if finalLen > ws.maxPacketLength:
    raise newException(WebSocketOversizedPacketError, "Packet too large")
